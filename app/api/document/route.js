...
export async function PUT(request) {
  ...
  try {
    ...
    const documentData = type === 'birth_certificate' ? { birth_certificate: documentUrl, birth_certificate_status: 1 } :
                          type === 'grade_card' ? { grade_card: documentUrl, grade_card_status: 1 } :
                          { good_moral: documentUrl, good_moral_status: 1}

    console.log(documentData)
        
    const { data: documentUpdatedData, error: documentError } = await supabase.from("Documents").update(documentData).eq("uid", id).select()
