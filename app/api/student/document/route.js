...
export async function PUT(request) {
	console.log("[DOCUMENT] 🚀 Starting update process...")

	try {
		// Parse FormData instead of JSON to handle file uploads
		console.log("[DOCUMENT] 📝 Parsing FormData...")
		...
		if (document && document.size > 0) {
			console.log("[DOCUMENT] 📸 Processing document upload...")
			const fileExtension = document.name.split(".").pop()
			const fileName = `${document.name}-${id}-${Date.now()}.${fileExtension}`

			console.log("[DOCUMENT] 📁 Uploading file")
			...
			if (uploadError) {
				console.error("[DOCUMENT] ❌ Document upload error:", uploadError)
				// Don't fail registration if document upload fails
			} else {
				console.log("[DOCUMENT] ✅ Document uploaded successfully")
			...
			console.log("[DOCUMENT] 📸 No document provided")
		}

		// Insert user document data into Document table
		console.log("[DOCUMENT] 💾 Updating user document data into database...")
		const documentData =
		...
		if (documentError) {
			console.error("[DOCUMENT] ❌ Document creation error:", documentError)
			...
			console.log("[DOCUMENT] ✅ Document data inserted successfully")
		}
		...
		console.log("[DOCUMENT] 📤 Sending response:", {
		...
		console.error("[DOCUMENT] 💥 Unexpected error during update:", error)
